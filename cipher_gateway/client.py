...
class CipherGatewayClient:
    ...
    async def start(self):
        ...
        self._started = True
        logger.debug("CipherGatewayClient started — %s", self._config.base_url)
    ...
    async def create_user(self) -> UserCredentials:
        """
        Create a new gateway user. No authentication required.
    ...
    async def get_symbol_price(self, symbol: str) -> SymbolPrice:
        ...
        Raises:
            CipherGatewayError: No price available (not subscribed, bad symbol).
        """
        try:
